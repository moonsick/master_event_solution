...
// 로그인 쿼리
exports.login="select * from admin_account where id=? and password=?";

//진행 행사 이벤트 list
exports.new_event_list="select * from event_M where isnull(is_ended) and isnull(is_del) order by event_ID desc";
//지난 행사 이벤트 list
exports.old_event_list="select * from event_M where !isnull(is_ended) and isnull(is_del) order by event_ID desc";


// 저장 버튼 누를시 이벤트 행사 신규 등록
exports.event_insert="insert into event_M select (select (count(event_M.event_ID)+1) from event_M),?,?,?,?,?,?, null, null";



// 수정 버튼 누를시 이벤트 행사 수정 등록
exports.event_amend="update event_M set event_name=? , event_start=? , event_end=? , "+
"event_commnet=? , event_location=? , event_company=? where event_ID=?;";


// 종료 버튼 누를시 동작 함수
exports.event_end="update event_M set is_ended='1' where event_ID=?";



// 삭제 버튼 누를시 동작 함수
exports.event_delete="update event_M set is_del='1' where event_ID=?";



// 강의 목록 list
exports.event_schedule_list="select * from event_schedule_D where event_ID=? and isnull(is_del) order by schedule_seq";


// 강연자 이름만 출력
exports.teacher_name_list="select * from user_D where event_ID=? and user_class_ID=2 and isnull(user_call) and isnull(is_del)";



// 저장 누를시 일정 추가 등록 eventScheduleInsert
exports.eventScheduleInsert="insert into event_schedule_D select ?,2,?,(select count(schedule_ID)+1 from event_schedule_D where event_ID=?),?,?,?,null";



// 수정 버튼 누를시 일정 등록 수정 eventScheduleMod
exports.eventScheduleMod="update event_schedule_D set schedule_seq=? , lecture=? where event_ID=? and user_class_ID=2 and user_ID=? and schedule_ID=?";



// 삭제 버튼 누를시 일정 등록 수정 eventScheduleDelete
exports.eventScheduleDelete="update event_schedule_D set is_del=1 where event_ID=? and user_class_ID=2 and user_ID=? and schedule_ID=?";



// 안내글 list noticeList
exports.noticeList="select * from notice_D where event_ID=? and isnull(is_del) order by notice_ID desc limit 1"



//신규 안내글 등록 하기 noticeInsertGo
exports.noticeInsertGo="insert into notice_D select ?,(select count(notice_ID)+1 from notice_D where event_ID=?),?,null";




// 안내글 수정 하기 noticeModGo
exports.noticeModGo="update notice_D set notice_comment=? where event_ID=? and notice_ID=?"



// 안내글 등록 삭제 버튼 noticeDeletgo
exports.noticeDeletgo="update notice_D set is_del=1 where event_ID=?"




//행사 클릭시 강연자 list teacherList
exports.teacherList="select * from user_D where event_ID=? and user_class_ID=1 and isnull(is_del)"



//행사 클릭시 참가자 list entryList
exports.entryList="select * from user_D where event_ID=? and user_class_ID=2 and isnull(is_del)"


// 강연자 신규 추가 등록 teacherInsert
exports.teacherInsert="insert into user_D select ?,1,(select count(user_ID)+1 from user_D where event_ID=? and user_class_ID=1),null, " +
"?,?,?,null";



// 강연자 수정 등록 teacherMod
exports.teacherMod="update user_D set user_name=? , user_party=? , user_call=? where event_ID=? and user_class_ID=? and user_ID=?";



// 강연자 삭제 등록 teacherDelete
exports.teacherDelete="update user_D set is_del=1 where event_ID=? and user_class_ID=? and user_ID=?";



// 참가자 신규 추가 등록 entryInsert
exports.entryInsert="insert into user_D select ?,2,(select count(user_ID)+1 from user_D where event_ID=? and user_class_ID=2),null, " +
"?,?,?,null";


// 참가자 수정 등록 entryMod
exports.entryMod="update user_D set user_name=? , user_party=? , user_call=? where event_ID=? and user_class_ID=? and user_ID=?";




// 참가자 삭제 등록 entryDelete
exports.entryDelete="update user_D set is_del=2 where event_ID=? and user_class_ID=? and user_ID=?";




// 진행+지난 행사 클릭시  함수 cardList
exports.cardList="select * from user_D where event_ID=? and isnull(is_del) and user_class_ID!=3";




//카드 저장 누를시 cardMod
exports.cardMod="update user_D set NFC_LID =?, user_name=?, user_party=? where event_ID=? and user_class_ID=? and user_ID=?";
